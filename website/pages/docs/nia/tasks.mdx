...
For complete information on the event structure, see [events in our API documentation](/docs/nia/api#events-1). Event information can be retrieved by using the [`include=events` parameter](/docs/nia/api#include) with the [task status API](/docs/nia/api#task-status).
