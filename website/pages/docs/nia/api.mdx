...
* `include` - `(string)` Only accepts the value "events". Use to request including stored event information with response.
...
* `services` - `([]string)` List of the services configured for the task.
* `providers` - `([]string)` List of the providers configured for the task.
* `events_url` - `(string)` Relative URL to retrieve the event data stored for the task.
* `events` - `([]struct)` - List of stored events that inform the task's status. See section below for information on event data. This field is only included in the response upon request by setting the `?include=events` parameter. The relative URL for the request to include events can be retrieved from the `events_url` field.
...
* `task_name` - `(string)` Name that task is configured with in Consul-Terraform-Sync.
* `error` - `(struct)` Information when the event fails. Null when successful.
